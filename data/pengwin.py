...
To complement the missing bone labels in original PENGWIN dataset,
...
PENGWIN_CLS_SET = {
    "background": frozenset([0]),
    "sacrum fragments": frozenset(range(1, 10+1)),
    "left ilium fragments": frozenset(range(11, 20+1)),
    "right ilium fragments": frozenset(range(21, 30+1))
}
SPLIT_JSON = os.path.join("datalist", "splitting-pengwin.json")
...
def get_slice_list(subset, data_root="~/data/pengwin"):
    assert subset in ("training", "test", "validation"), subset
    split_csv_f = os.path.join("datalist", f"datalist_pengwin_{subset}.csv")
    npz_list = []
    ...
    print("make data list: pengwin", subset)
...
def combine_n_binarise(label, ts_label, mode):
    """combine original PENGWIN label & TotalSegmentator prediction"""
    assert mode in ("partial", "full", "as-is"), mode
    # map pengwin label to TotalSegmentator
    lab_p2ts = np.zeros_like(label, dtype=np.int32)
    lab_p2ts[(1 <= label) & (label <= 10)] = 25 # sacrum
    lab_p2ts[(11 <= label) & (label <= 20)] = 77 # left hip
    lab_p2ts[(21 <= label) & (label <= 30)] = 78 # right hip
    # remove pelvic (sacrum & hip) from TS prediction
    ts_lab_exc = unlabel(ts_label, TOTALSEG_CLS_SET["pelvic"])
    # combine
...
def iter_vol_dataset(
    ...
    window=True, window_level=[300], window_width=[290], data_root="~/data/pengwin"
):
